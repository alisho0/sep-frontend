  ...
  const handleDeleteCiclo = async (id) => {
    ...
    try {
      ...
      if (eliminarCiclo.fulfilled.match(delCiclo)) {
        ...
        throw new Error(
          "El ciclo no pudo ser eliminado correctamente. Intentalo de nuevo."
        );
      }
    } catch (e) {
      showAlert({
  ...
  return (
    <>
      <div className="container mx-auto md:px-28 px-4 pt-9">
        <div>
          <button
            className="col-span-1 md:col-span-3 text-left flex gap-2 items-center hover:bg-indigo-700 hover:text-white mb-3 w-fit py-1 px-2 rounded-lg transition-colors cursor-pointer"
            onClick={() => navigate(-1)}
          >
        ...
        <div className="mt-6">
          ...
          <div className="flex gap-3 border-b border-gray-400 mb-6">
            ...
            {gradoActual.seccionCiclos.map((sc, idx) => (
              <button
                key={idx}
                className={`p-4 font-semibold hover:text-black transition ${
                  seccionSelected === idx
          ...
          <div className="bg-white p-6 rounded-lg shadow-md border-gray-300">
            ...
            {gradoActual.seccionCiclos[seccionSelected]?.gradoCiclos?.length >
            0 ? (
              <>
                {gradoActual.seccionCiclos[seccionSelected]?.gradoCiclos.map(
                  (ciclo) => (
                    <div
                      key={ciclo.id}
                    ...
                    >
                      ...
                      <div className="flex md:flex-row flex-col md:mt-0 mt-2 gap-2">
                        <Link to={`/grados/${id}/ciclo/${ciclo.id}`} className="flex items-center justify-center gap-2 bg-indigo-600 hover:bg-indigo-700 transition-colors text-white font-semibold rounded-lg cursor-pointer">
                          <BotonIcono
                            Icono={EyeIcon}
